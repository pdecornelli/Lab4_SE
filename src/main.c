...
int main(void) {
	...
	BSP_Init();
	BSP_LED_Init(LED_AZUL);
	BSP_LED_Init(LED_VERDE);
	BSP_LED_Init(LED_ROJO);
	BSP_LED_Init(LED_NARANJA);
	...
	xTaskCreate(Led_Parametro, (signed char * ) "TaskB",
			configMINIMAL_STACK_SIZE, (void * ) 1, tskIDLE_PRIORITY,
			( xTaskHandle * ) NULL);
